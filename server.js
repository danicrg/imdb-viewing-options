...
app.post("/", async (req, res) => {
	try {
		...
		const list = await Promise.all(mergedResult.map(movie => fetchMovieDetails(movie)));
		const listWithViewingOptions = list.filter(item => item.viewingOptions.length > 0);

		listWithViewingOptions.sort((a, b) => (b.metascore || 0) - (a.metascore || 0));
